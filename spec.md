...
| 7      | AI Interview + i18n + Settings | **AI Interview:** `interview_plan.rs` generates question plans from job descriptions via BYOK LLM. `interview_runner.rs` orchestrates the AI interview flow (emits `interview-question`, `interview-status`, `interview-finished` events). `tts/mod.rs` wraps macOS `say` for text-to-speech. `LiveInterview` + `PlanGenerator` React components in `App.tsx`. `start_ai_interview`/`skip_ai_question`/`stop_ai_interview` Tauri commands. **i18n:** `src/i18n.ts` with 142 bilingual EN/ES translation keys, `useLanguage()` hook, language switcher in `Header.tsx`, persisted choice (localStorage + backend `settings.language`). `Icon.tsx` (24 inline SVG icons), `ui.tsx` (shared primitives), `hooks.ts` (custom hooks including `useLLMSettings` for per-feature provider/model defaults), `validation.ts` (path/error helpers), `constants.ts` (provider list), `ApiKeyInput.tsx`. **Settings:** `SettingsDialog.tsx` with 3-tab dialog (API Keys, LLM Defaults, General), `Header.tsx` settings button, `keys::delete_key`/`keys::list_saved_keys` Tauri commands for key management. Onboarding expanded to 4 steps (added API key step). `get_log_dir_path` command for log viewing via frontend. DeepSeek provider added to BYOK providers list. | ✅ **Completed** |
