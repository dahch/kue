...
## 9. User flow
 ...
 1. **Initial setup (first launch only):** After Moonshine provisioning completes (`ProvisioningProgress` → `moonshine-provisioned` event), the frontend checks `is_first_run` (reads `settings.first_run` from DB). If `pending` or missing, it shows the **Onboarding wizard** (`src/Onboarding.tsx`) which guides the user through three steps before the main app unlocks:
    ...
    - **Embedding model loading:** polls `is_embedding_model_loaded` (checks Tauri state for the `EmbeddingModel` instance) at 1s intervals while the model is being loaded synchronously during app setup. Shows a spinner with "Cargando modelo" text.
    - **Folder selection for indexing:** the user enters an absolute path to their projects/CV folder. The `index_folder_cmd` Tauri command indexes all PDF/TXT/MD files into the RAG database. A "Saltar" (skip) button is provided. On completion, `mark_onboarding_done` sets `settings.first_run = 'done'` so subsequent launches skip the wizard and go directly to `MainApp`.
