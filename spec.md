...
| 7      | AI Interview + i18n  | **AI Interview:** `interview_plan.rs` generates question plans from job descriptions via BYOK LLM. `interview_runner.rs` orchestrates the AI interview flow (emits `interview-question`, `interview-status`, `interview-finished` events). `tts/mod.rs` wraps macOS `say` for text-to-speech. `LiveInterview` + `PlanGenerator` React components in `App.tsx`. `start_ai_interview`/`skip_ai_question`/`stop_ai_interview` Tauri commands. **i18n:** `src/i18n.ts` with 270+ bilingual EN/ES translation keys, `useLanguage()` hook, language switcher in `Header.tsx`, persisted choice (localStorage + backend `settings.language`). `Icon.tsx` (30 inline SVG icons), `ui.tsx` (shared primitives), `hooks.ts` (custom hooks), `validation.ts` (path/error helpers), `ApiKeyInput.tsx`. Onboarding expanded to 4 steps (added API key step). `get_log_dir_path` command for log viewing via frontend. DeepSeek provider added to BYOK providers list. | ✅ **Completed** |
