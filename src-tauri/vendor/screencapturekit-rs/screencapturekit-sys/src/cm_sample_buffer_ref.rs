    ...
    pub fn get_frame_info(&self) -> Id<SCStreamFrameInfo> {
        unsafe {
            ...
            if raw_attachments_array.is_null() {
                let empty_dict: *mut Object = msg_send![class!(NSMutableDictionary), new];
                return Id::from_retained_ptr(empty_dict.cast());
            }
            let first: *mut Object = msg_send![raw_attachments_array, firstObject];
            if first.is_null() {
                let empty_dict: *mut Object = msg_send![class!(NSMutableDictionary), new];
                return Id::from_retained_ptr(empty_dict.cast());
            }
            Id::from_ptr(first.cast())
        }
    }
