...
│  │ │UI)     │ │ │  - audio::capture::is_transcript│    │
│  │ └────────┘ │ │    _ready cmd                   │    │
│  │            │ │  - audio::mic_vad (MicVadState) │    │
│  │ ┌────────┐ │ │  - rag::index_folder_cmd        │    │
│  │ │ Overlay│ │ │  - rag::search_context cmd      │    │
│  │ │(hint   │ │ │  - overlay::show_overlay cmd   │    │
│  │ │ window)│ │ │  - stt::pipeline (VAD→STT→     │    │
│  │ └────────┘ │ │    classify→events→DB)          │    │
│  │            │ │  - classifier::classify_text    │    │
│  │ React +    │ │    (question classification)    │    │
│  │ Tailwind   │ │  - orchestrator (HintScheduler+ │    │
│  │            │ │    PanicState)                  │    │
│  │            │ │  - types (TranscriptLine,       │    │
│  │            │ │    Speaker + Copy)              │    │
│  │            │ │  - analyze (post-call BYOK      │    │
...
│   │   │   │                #   panic_mode/is_transcript_ready cmds, spawn_batch_transcription
...
│   │   │                    #   trap keywords for regret/failure, 76 tests)
│   │   ├── orchestrator/
│   │   │   ├── mod.rs       # HintJob, HintScheduler, PanicState, should_cancel_hint,
│   │   │   │                #   generate_and_emit_hint, emit_expired_hints (73 tests)
│   │   │   └── worker.rs    # hint worker thread (poll loop + expired hint emission)
│   │   ├── stt/
│   │   │   ├── mod.rs       # STTEngine trait, STTConfig, persist_transcript_line, create_engine
